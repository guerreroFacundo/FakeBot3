...
class steam:
    ...
    def serch_player(player: str):
        ...
        try:
            ...
            if (data == "No match"):
                embed = discord.Embed(
                    title=f"No tenes URL personalizada en tu perfil, ingresa aca para hacerlo: https://steamcommunity.com/id/{player}/edit/info/info",
                    description="", color=colores)
            else:
                            ...
                            if 'price_overview' in game_data['data']:
                                ...
                                if game_data['data']['price_overview']['discount_percent'] != 0:
                                    discount_percent = "%" + str(
                                        game_data['data']['price_overview']['discount_percent'])
                                print(
                                    f"El precio final formateado para el juego con ID {game_id} es: {precioofthisgame}")
                            else:
                            ...
                            for datito in datavalor:
                                for datito2 in datito:
                                    if (datito2 == "data"):
                                        ...
                                        embed.add_field(name=f"Juego recientemente jugado: ",
                                                        value=f"Nombre: {game['name']} \n Tiempo de juego: {int(game['playtime_forever']) / 60}  \n Precio: {precioofthisgame} \n Descuento: {discount_percent} \n Precio final: {preciofinalofthisgame} ",
                                                        inline=False)

                except Exception as err:
                    print("no se encontraron juegos recientes")

                embed.add_field(name=f"URL personalizada: ", value=f"https://steamcommunity.com/id/{player}",
                                inline=True)
